...
//---------------------------------------------------------------------------------
function DeleteHubspotModal({
  selectedHubspot,
  showDeleteHubspot,
  setShowDeleteHubspot,
}) {
  ...
  const deleteCrm = async () => {
    ...
    await axios
      .delete(ApiLinks.Hubspot.Delete + selectedHubspot, {
        headers: {
  ...
  return (
    <Modal
      as={Modal.Dialog}
      centered
      show={showDeleteHubspot}
      onHide={() => setShowDeleteHubspot(false)}
    >
      <Modal.Header>
        <Button
          variant="close"
          aria-label="Close"
          onClick={() => setShowDeleteHubspot(false)}
        />
      </Modal.Header>
      <Modal.Body>
        <h5 className="mb-4">Confirm deleting this hubspot ?</h5>
      </Modal.Body>
...
export default DeleteHubspotModal;
