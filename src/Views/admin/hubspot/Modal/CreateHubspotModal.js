...
//---------------------------------------------------------------------
function CreateHubspotModal({ showCreateHubsport, setShowCreateHubsport }) {
  ...
  const onSubmit = async (event) => {
    ...
    setApiLoading(true);
    await axios
      .post(ApiLinks.Hubspot.Create, newCrm, {
        headers: {
          Authorization: `Bearer ${Token}`,
        },
      })
      .then((res) => {
        setShowCreateHubsport(false);
        setBackErrors({});
  ...
  return (
    <Modal
      as={Modal.Dialog}
      centered
      show={showCreateHubsport}
      onHide={() => setShowCreateHubsport(false)}
    >
      <Modal.Header>
        <Button
          variant="close"
          aria-label="Close"
          onClick={() => setShowCreateHubsport(false)}
        />
      </Modal.Header>
      <Modal.Body>
        <h5 className="mb-4">Create a hubspot</h5>
...
export default CreateHubspotModal;
